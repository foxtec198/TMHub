...
function state(agent) {
  ...
  return agent.online
    ? { label: "Online", severity: "success" }
    : { label: "Offline", severity: "warning" };
}
...
export function RpaAgents() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        "error",
        "Agentes RPA",
        errorMessage(error, "Não foi possível consultar os agentes.")
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const target = Math.max(
      0,
      Math.min(100, Number(execution.progress) || 0)
    );
  ...
  useEffect(() => {
    ...
    const progress = (event) => {
      ...
      const completed =
        event.status === "completed" || Number(event.progress) >= 100;
      ...
      setExecution((current) => {
        if (
          current?.commandId !== event.command_id ||
  ...
  const runJornadas = async () => {
    try {
      setExecution({
        progress: 2,
        step: "Enviando comando ao agente",
        status: "running",
      });
      ...
      setExecution({
        ...
        progress: 5,
        step: "Agente recebeu a solicitação",
        status: "running",
      });
    } catch (error) {
  ...
  const visibleAgents = category
    ? agents.filter((agent) => agent.category === category)
    : agents;

  const groups = visibleAgents.reduce(
    (result, agent) => ({
      ...result,
      [agent.category]: [
  ...
  return (
    <section className="rpa-agents-page">
      <PageHeader
        section="RPA Center"
        title={
          category
        ...
        description="Agentes locais que executam tarefas integradas ao TMHub."
      />

      {loading ? (
        <div className="rpa-agents-loading">
          Carregando automações…
        </div>
      ) : (
        Object.entries(groups).map(([group, items]) => (
          <section className="rpa-agent-group" key={group}>
            <header>
              <div>
                <span>{group}</span>
              ...
              <small>
                {items.length} disponível(is) neste departamento
              </small>
            </header>

            <div className="rpa-agent-list">
              {items.map((agent) => {
                const status = state(agent);
                const details = automationDetails(agent);

                const isPontoMais =
                  agent.capability === "pontomais_report_import";
                ...
                const running =
                  isPontoMais &&
                  (execution?.status === "running" || finishing);

                const progress = isPontoMais ? execution : null;

                return (
                  <article
                    className={`rpa-agent-row ${agent.active ? "" : "is-inactive"
                      }`}
                    key={agent.key}
                  >
                    <div className="rpa-agent-row__identity">
                      <span className="rpa-agent-icon">
                        <AppIcon name={details.icon} />
                      </span>
                    ...
                    <dl className="rpa-agent-facts">
                      ...
                      <div>
                        <dt>Máquinas</dt>
                        <dd>
                          {agent.machines?.length || 0} conectada(s)
                        </dd>
                      </div>
                    </dl>

                    <div className="rpa-agent-row__action">
                      {isPontoMais ? (
                        <Button
                          label={
                            running
                    ...
                    {isPontoMais && progress && (
                      <div
                        className={`rpa-agent-progress is-${progress.status}`}
                        aria-live="polite"
                      >
                        <div className="rpa-agent-progress__meta">
                          <span>{progress.step}</span>
                          <strong>{displayProgress}%</strong>
                        </div>

                        <div
                          className="rpa-agent-progress__track"
                          role="progressbar"
                          aria-valuemin="0"
                          aria-valuemax="100"
                          aria-valuenow={displayProgress}
                          aria-label="Andamento da automação"
                        >
                          <i
                            style={{
            ...
            {/* Loading da automação */}
            {isRunning && (
              <div className="rpa-loading">
                <video
                  src="/rpa_loading.webm"
                  autoPlay
                  loop
                  muted
                  playsInline
                ...
                <span>
                  {execution.step || "Executando automação..."}
                </span>
              </div>
            )}
          </section>
