...
```powershell
npm.cmd run work:new -- -Type feat -Name "central de chamados"
```
...
```powershell
npm.cmd run deploy:pr -- -Message "feat: cria central de chamados"
```
...
> `deploy:pr` não faz deploy em produção. As branches `main` e `dev` são
